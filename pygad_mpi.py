...
class GA(utils.parent_selection.ParentSelection,
    ...
    def __init__(self,
        ...
        try:
            ...
            # A list hodling the mutated offspring's fitness
            self.last_generation_mutation_fitness = None
            
            if rank !=0:
                self.valid_parameters = None
             
            # Check if the parallel_processing is correct.
    ...
    def cal_pop_fitness(self):
        ...
        try:
            if rank == 0:
                ...
                if self.last_generation_offspring_mutation is not None:
                    last_generation_offspring_mutation_as_list = [
                        list(offspring) for offspring in self.last_generation_offspring_mutation]
            ...
            else: # self.parallel processing is not None
                if rank == 0:
                    # Calculating the fitness value of each solution in the current population.
                    for sol_idx, sol in enumerate(self.population):
                        # Check if the `save_solutions` parameter is `True` and whether the solution already exists in the `solutions` list. If so, use its fitness rather than calculating it again.
                        ...
                        # elif (self.last_generation_parents is not None) and len(numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0] > 0):
                            ...
                            fitness = self.previous_generation_fitness[parent_idx]    
                            pop_fitness[sol_idx] = fitness
                        # If the mutation operation already computed the fitness of the mutated offspring, these valuese are stored in last_generation_mutation_fitness
                        elif (self.last_generation_mutation_fitness is not None) and (list(sol) in last_generation_offspring_mutation_as_list):
                            # Find the index of the offspring in the list of offspring.
                            offspring_idx = last_generation_offspring_mutation_as_list.index(list(sol))
                            # Use the returned offspring index to return its pre-calculed fitness value.
                            fitness = self.last_generation_mutation_fitness[offspring_idx]
                            pop_fitness[sol_idx] = fitness
                            
                            
                comm.barrier()
                # Process 0 determines which individuals need processing, if any
                len_sol_to_sub = None
                if rank == 0:
                    ...
                    for sol_idx, sol in enumerate(self.population):
                        ...
                        if type(pop_fitness[sol_idx]) is str:
                            ...
                            pass
                    len_sol_to_sub = len(solutions_to_submit)
                else: # Process none 0 have to wait
                    pass
                comm.barrier()
                len_sol_to_sub = comm.bcast(len_sol_to_sub, root=0) # Values used to know whether a process will receive a tzsk
                comm.barrier()
                
                # Check if there values to compute, else pass.                
                if len_sol_to_sub != 0:                
                    # Use MPI to evalutate the population in parallel    
                    if rank == 0:
                        task_per_worker = len_sol_to_sub // (size-1)
                        extra_tasks = len_sol_to_sub % (size-1)
                        # Send the population to different processes
                        start_idx = 0
                        start_idx_list = [0 for _ in range(len_sol_to_sub+1)]
                        for i in range(1, min(size-1,len_sol_to_sub)+1):
                            end_idx = start_idx + task_per_worker + (1 if i <= extra_tasks else 0)
                            comm.send(solutions_to_submit_indices[start_idx:end_idx], dest=i, tag=0)
                            comm.send(solutions_to_submit[start_idx:end_idx], dest=i, tag=1)
                            start_idx = end_idx
                            start_idx_list[i] = start_idx
                        # Receive the value computed by other processes
                        for _ in range(1, min(size-1,len_sol_to_sub)+1):
                            status = MPI.Status()
                            fit_list = comm.recv(source=MPI.ANY_SOURCE, tag=2, status=status)
                            worker_rank = status.Get_source()
                            if fit_list is not None:
                                idx_list=range(start_idx_list[worker_rank-1],start_idx_list[worker_rank])
                                for j,idx in enumerate(idx_list):
                                    pop_fitness[solutions_to_submit_indices[idx]] = fit_list[j]
                    elif rank < min(size-1,len_sol_to_sub)+1: # Process that have received a task
                        sol_idx = comm.recv(source=0, tag=0)
                        sol = comm.recv(source=0, tag=1)
                        tasks = zip(sol_idx, sol)
                        fit_list=[self.fitness_func(self,s,idx) for idx, s in tasks]
                        comm.send(fit_list, dest=0, tag=2)
                    
                    else: # Process that have nothing do to
                        pass                
                comm.barrier()
            if rank == 0:
                ...
                pop_fitness = None
            # Empty last_generation_muation_fitness so that these values don't interfere with future calls to cal_pop_fitness
            self.last_generation_mutation_fitness = None
    ...
    def run(self):
        ...
        try:
            ...
            if rank == 0:
                ...
                generation_first_idx = None
                generation_last_idx = None
            
            comm.barrier()
            generation_first_idx = comm.bcast(generation_first_idx, root=0)
            generation_last_idx = comm.bcast(generation_last_idx, root=0)            
            comm.barrier()
            ...
            for generation in range(generation_first_idx, generation_last_idx):
                ...
                comm.barrier()                            
                stop_run = comm.bcast(stop_run, root=0) 
                comm.barrier()
                if stop_run:
                    break
